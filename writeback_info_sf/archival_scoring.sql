-- =============================================================================
-- archival_scoring.sql (Snowflake)
--
-- Before running, replace <YOUR_DATABASE> and <YOUR_SCHEMA> below with the
-- Snowflake database and schema where SIGDS_WORKBOOK_MAP resides.
-- =============================================================================
...
-- 3. SIGDS table modification       15    Days since last alteration to the data table
--                                         NOTE: SIGDS_TABLE_LAST_MODIFIED reflects
--                                         LAST_ALTERED from INFORMATION_SCHEMA.TABLES
--                                         which tracks DDL operations in Snowflake.
--                                         A recently-altered SIGDS table may reflect
--                                         a schema change, not user writeback activity.
--                                         Cross-check with WAL edit recency before
--                                         drawing conclusions.
...
--      e.g. ALTER TABLE <DATABASE>.<SCHEMA>.<TABLE>
...
WITH scored AS (
    ...
    SELECT
        ...
        --    Measures data-layer activity via LAST_ALTERED from
        --    INFORMATION_SCHEMA.TABLES. NULL treated as > 365 days.
        -- -----------------------------------------------------------------
        CASE
