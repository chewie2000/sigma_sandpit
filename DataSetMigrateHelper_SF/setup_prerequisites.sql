...
--
-- One-time setup script for the DataSetMigrateHelper_SF toolkit.
...
CREATE OR REPLACE NETWORK RULE sigma_api_network_rule
  ...
  -- Example: VALUE_LIST = ('api.eu.aws.sigmacomputing.com:443');


-- ==============================================================================
-- 2. Secrets — store Sigma API credentials outside of procedure code
--    Credentials are never visible in procedure source or query history.
--    Use CREATE OR REPLACE only if you need to rotate credentials.
-- ==============================================================================
...
--    References both the network rule and the secrets so procedures can
--    make outbound API calls and read credentials at runtime.
-- ==============================================================================

CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION sigma_api_access
  ALLOWED_NETWORK_RULES          = (sigma_api_network_rule)
  ALLOWED_AUTHENTICATION_SECRETS = (sigma_client_id, sigma_client_secret)
  ENABLED = TRUE;


-- ==============================================================================
-- 4. Grants — allow the procedure-execution role to use the integration
--    and read the secrets.
